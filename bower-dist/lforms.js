    ...
    _updateAutocompOptions: function _updateAutocompOptions(item, forceReset) {
      ...
      if (item.dataType === this._CONSTANTS.DATA_TYPE.CNE || item.dataType === this._CONSTANTS.DATA_TYPE.CWE) {
        ...
        if (url) {
          ...
          if (!this.hasSavedData && !options.defaultValue && options.listItems.length === 1) options.defaultValue = options.listItems[0]; // If this is a saved form with user data, and this field has not yet
          ...
          // changed.)
          else if (this.hasSavedData && item.value && !item._hasHadNonEmptyList) options.defaultValue = item.value.code ? {
