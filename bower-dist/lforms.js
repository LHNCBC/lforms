...
module.exports = JSON.parse("{\"lformsVersion\":\"29.1.3\"}");
...
angular.module('lformsWidget').controller('LFormsCtrl', ['$window', '$scope', '$element', '$timeout', '$interval', '$sce', 'smoothScroll', 'LF_CONSTANTS', 'lformsConfig', function ($window, $scope, $element, $timeout, $interval, $sce, smoothScroll, LF_CONSTANTS, lformsConfig) {
  ...
  */
  ...
  $scope.createAttachment = function (item) {
    ...
    item.value = {
      title: item._attachmentName || ((_item$_fileInfo = item._fileInfo) === null || _item$_fileInfo === void 0 ? void 0 : _item$_fileInfo.name) || 'Unknown filename'
    };
      ...
      if (item._skipLogicStatus !== this._CONSTANTS.SKIP_LOGIC.STATUS_DISABLED) {
        ...
        if (angular.isArray(item.value) && item.value.length > 0) {
          ...
          isEmpty = !notEmpty;
            ...
            isEmpty = !item.value.title && !item.value.url && (item.value.text === undefined || item.value.text === null || item.value.text === "");
          } // simple type
