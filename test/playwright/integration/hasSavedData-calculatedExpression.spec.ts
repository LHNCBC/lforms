...
test.describe('calculatedExpression and hasSavedData=true/false tests', () => {
  ...
  function readQuestionnaire() {
    const absPath = path.resolve('test/data/R4/questionnaire-calculatedExpression.json');
    return JSON.parse(fs.readFileSync(absPath, 'utf-8'));
  }

  test('should have calculated values', async ({ page }) => {
  ...
  test('should have the user saved data, instead of the calculated values even if the user saved data are empty strings in QuestionnaireResponse', async ({ page }) => {
    await openPageAndLoadForm(page);
  ...
  test('should have the user saved data, instead of the calculated values if there are saved user data in QuestionnaireResponse', async ({ page }) => {
    await openPageAndLoadForm(page);
    const qr = await getQR(page);
    const q = readQuestionnaire();
