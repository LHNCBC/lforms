...
describe('Question with sub items', () => {
  ...
  it('should render sub items for each selected checkbox and export properly', () => {
    ...
    cy.byId('label-multi-select-subgroup||a/1/multi-select-subgroup||a').should('be.visible');
    cy.byId('label-multi-select-subgroup||b/1/multi-select-subgroup||b').should('be.visible');
    cy.byId('label-multi-select-subgroup||o/1/multi-select-subgroup||o').should('be.visible');
    // Unchecking an option should remove the sub items for that option.
    cy.byId('parent-checkbox/1||o').click();
    cy.byId('label-multi-select-subgroup||o/1/multi-select-subgroup||o').should('not.exist');
    // Fill out the sub items.
    cy.byId('child-integer/1/multi-select-subgroup||a/1').type('11');
    cy.byId('child-integer/1/multi-select-subgroup||b/1').type('22');
    cy.window().then((win) => {
      ...
      cy.byId('label-multi-select-subgroup||a/1/multi-select-subgroup||a').should('not.exist');
      win.LForms.Util.addFormToPage(mergedFormData, 'formContainer');
      cy.byId('label-multi-select-subgroup||a/1/1').should('be.visible');
      cy.byId('label-multi-select-subgroup||b/1/1').should('be.visible');
      cy.byId('item-multi-select-subgroup||a/1/1').byId('child-integer/1/1/1').should('have.value', 11);
      cy.byId('item-multi-select-subgroup||b/1/1').byId('child-integer/1/1/1').should('have.value', 22);
    });
  });
  ...
  it('should remove invalid sub items when options are removed by answerExpression', () => {
    ...
    cy.byId('label-multi-select-subgroup||Apple/1/multi-select-subgroup||Apple').should('be.visible');
    cy.byId('label-multi-select-subgroup||Banana/1/multi-select-subgroup||Banana').should('be.visible');
    // Fill out sub items for Apple and Banana.
    cy.byId('child-integer/1/multi-select-subgroup||Apple/1').type('11');
    cy.byId('child-integer/1/multi-select-subgroup||Banana/1').type('22');
    ...
    cy.byId('label-multi-select-subgroup||Banana/1/multi-select-subgroup||Banana').should('not.exist');
  ...
  it('should load back merged QR, when checkbox options are set by answerExpression', () => {
    ...
    cy.window().then((win) => {
      ...
      const q = win.LForms.Util.getFormFHIRData("Questionnaire", "R4");
      const qr = win.LForms.Util.getFormFHIRData("QuestionnaireResponse", "R4");
      expect(q.item[1].item.length).to.equal(1);
      expect(q.item[1].item[0]).to.deep.equal({
        "type": "integer",
        "linkId": "child-integer",
        "text": "How many of this?"
      });
      expect(qr.item[1].answer.length).to.equal(2);
      expect(qr.item[1].answer[0]).to.deep.equal({
        "valueCoding": {
          "display": "Apple"
        },
        "item": [
      ...
      expect(qr.item[1].answer[1]).to.deep.equal({
        "valueCoding": {
          "display": "Banana"
        },
        "item": [
      ...
      const mergedFormData = win.LForms.Util.mergeFHIRDataIntoLForms(qr, formDef, 'R4');
      win.document.getElementById('formContainer').innerHTML = null;
      cy.byId('label-multi-select-subgroup||Apple/1/multi-select-subgroup||Apple').should('not.exist');
      win.LForms.Util.addFormToPage(mergedFormData, 'formContainer');
      cy.byId('label-multi-select-subgroup||Apple/1/1').should('be.visible');
      cy.byId('label-multi-select-subgroup||Banana/1/1').should('be.visible');
      cy.byId('item-multi-select-subgroup||Apple/1/1').byId('child-integer/1/1/1').should('have.value', 11);
      cy.byId('item-multi-select-subgroup||Banana/1/1').byId('child-integer/1/1/1').should('have.value', 22);
    });
  });
});
