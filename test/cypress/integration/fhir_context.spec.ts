    ...
    describe(fhirVersion, () => {
      ...
      describe('External autocomplete answerValueSets', () => {
        it('should be able to search via ValueSet expansion', () => {
          cy.byId(ff.ethnicity).click().type('arg');
          cy.get('#completionOptions ul li:first-child').should('have.text', 'Argentinean');

          cy.byId(ff.disease).click().type('arm');
          cy.get('#completionOptions ul li:first-child').click({force: true});
