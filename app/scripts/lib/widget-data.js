...
// A class for form definition data validation. (not for user data validation)
var FormValidation = Class.extend({


  //// Special variable naming convention for the form definition data and user data in "items"
  // variableName   : Public. It is visible and required in the form/user data
  // _variableName  : Private. It is not visible to outside developers for showing a form without existing user data.
  //                  But it is visible and required when showing a form along with previously saved user data.
  // _variableName_ : Private. Totally internal data for widget. Not visible at all.
  defaultItems : {
    "_id":"1",                 // initial value is 1, later calculated from questionCode and parentQuestionCode _id
    "_idPath":"/1",            // calculated from questionCode and parentQuestionCode and _id
    "_codePath":"/",           // calculated from questionCode and parentQuestionCode
    "_displayLevel_":1,      // internal, calculated from _codePath or _idPath
    "_parentIdPath_": "",    // internal, calculated from questionCode and parentQuestionCode, might needed for performance reasons
    "_parentCodePath_": "",  // internal, calculated from questionCode and parentQuestionCode, might needed for performance reasons
    "questionCode":"",
    "parentQuestionCode":"",  // not needed after the construction of _idPath and _codePath
    "header":false,
    "question":"",
    "codingInstructions":"",
    "formula":"",
    ...
    "units":"",
    "skipLogic":"",
    "layout":"",          // vertical/horizontal. '' means 'vertical', valid only when "header" is true. items within a horizontal table should not have sub items.
    "_inHorizontalTable_":false, // when the container section is a horizontal table, this field of each item (should be leaf item too) within the section should be true.
    "formatting":"",      // optional, may not needed
    "editable":"1",           // "0": not editable; "1": editable; "2" editable on new records, not editable on existing records
    "questionCardinality":{"min":1, "max":1},
    "answerCardinality":{"min":0, "max":1},
    "defaultAnswer":"",
    "answers":"",
    "externallyDefined":"",
    "_value":"",      // the saved user typed/picked value
    "_valueOther":"", // the 'other' value user typed when such an option is provided in the answer list
    "_unit":""        // the saved user picked unit
  },
  ...
  defaultUnit : {
    "name":"",
    "range":{"minInclusive":null, "maxInclusive":null},
    "default":false,
    "code":""
    //"codeSystem":""
  },
  ...
  defaultObrData : {
    ...
    "dataType":"ST",
    "answers":"",
    "formatting":{"width":"10em","min-width":"4em"},
    "answerCardinality":{"min":1, "max":1},
    "_value":""     // the saved user typed/picked value
  },

  // validation functions
  validateFormDefinitionData: function(formDefData) {
    /// all values are of type "string", unless otherwise stated. possible other types: array, hash, boolean, integer

    //// check form options
    // type: optional. but required for widget. it should be a permitted value if present, if not present, set it to the default value as "LOINC",
    // code: optional. it should be a permitted value if present.
    // name: required.
    // template: optional, but required for widget. it should be a permitted value if present, if not present, set it to the default value as "panelTable.html".

    //// check template option
    //// templateOption: for template "panelTable.html", everything is optional. Since it's the only template the widget supports and it is still under development, nothing need to set here for now.

    //// check items
    // the item for the form itself is not in the array
    // check the structural order of the items. children items appear after its parent item. the item position in the array is the same order for display.
    // siblings' questionCode values are unique

    //// reconstruct fields: _id, _idPath, _codePath, if they are not present, check the field value if they are present.
    //// note: _idPath and _codePath together are the unique identifier of each item, including the repeating items/groups
    // _id: digits. it should always be "1" for new records. value for repeating items/groups could be other digits, i.e. "1"
    // _idPath: digits and PATH_DELIMITER (default "/"), format: /[ancestor's _id]/../[parent's _id]/[_id]. i.e. "/1", "/1/1", "/1/2", or "/1/2/1"
    // _codePath: questionCode and PATH_DELIMITER (default "/"), format: /[ancestor's questionCode]/../[parent's questionCode]/[_id]. i.e. "/54126-8", "/54126-8/54125-0"


    //// for each question item in items
    // check no extra fields
    // check missing fields are allowed to be optional
    // reconstruct internal fields for widget: _parentIdPath_, _parentCodePath_, _displayLevel_

    //// for each field in item
    // questionCode: required, a permitted value for the specified form type
    // parentQuestionCode: required, a permitted value for the specified form type
    // header: optional, boolean, true/false, default false
    // question: required
    // codingInstructions: optional
    // formula: optional, hash, permitted keys in the hash: "name", "value". currently the permitted values for "name" are "BMI", "SCORE"
    //           "value" contains an array of _questionCode
    //    {
    //      name: 'BMI',
    //      value: ['/12345-1', '/23498-2']
    //    }
    //
    //          if formula is not embedded within the item data, and is in the separate formulas variable, a "target" key should be included:
    //    {
    //      name: 'BMI',
    //      value: ['/12345-1', '/23498-2'],
    //      target: '/38743-3'
    //    }
    //  dataType: required, a permitted value,
    //  restrictions: optional, array of hashes, permitted keys in the hash: "name", "value". currently the permitted values for "name" are:
    //        "minExclusive",
    //        "minInclusive",
    //        "maxExclusive",
    //        "maxInclusive",
    ...
    //      {"name": "pattern", "value":["URL","/^[a-zA-Z]+*/"}
    //    ]
    //  units: optional, array of hashes
    //    i.e.
    //    [
    //      {"name":"ft-inches","code":"1","default":true},
    //      {"name":"centimeters","code":"2"}
    //    ]
    //  skipLogic:
    //  formatting:
    //  editable: optional, permitted values are "0" (readonly), "1"(editable) and "2"(editable for new, readonly for existing data). default is "1",
    //  questionCardinality: optional, required by widget. default {"min":1, "max":1}
    //  answerCardinality: optional, required by widget. default {"min":0, "max":1}
    //  defaultAnswer: optional, the default user data, a value or an answer in answer lists, it could be any data type
    //  answers: optional, array of hashes. required for dataType "CNE" and "CWE" (it may be defined externally in next field, not implemented)
    //  externallyDefined: optional. for "CNE" and "CWE"

    //// check 2 additional fields if existing user data present
    // _value: the saved user data, check if the dataType match, if it's "CNE" or "CWE", check if the value is in the answer list
    // _unit: optional, the selected unit for the save data, if present, check if the selected unit is in the units list
    ...
    //// check if answerLists, skipLogicRules, formulas, restrictions and units exist separately.
