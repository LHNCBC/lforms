  ...
  self._processFHIRValue = function(lfItem, fhirVal, setDefault) {
    ...
    if (lfDataType === 'CWE' || lfDataType === 'CNE' ) {
      if (lfItem.answerCardinality && lfItem.answerCardinality.max === '*') {
        lfItem.value = [{code: fhirVal.code, text: fhirVal.display}];
        if (setDefault)
          lfItem.defaultAnswer = [{code: fhirVal.code, text: fhirVal.display}];
      }
      // single selection
      else {
        lfItem.value = {code: fhirVal.code, text: fhirVal.display};
      ...
      if (fhirVal.value !== undefined) {
        ...
        if (setDefault)
          lfItem.defaultAnswer = fhirVal.value;
      }
      let unit = fhirVal.code? fhirVal.code: fhirVal.unit;
      if (unit) {
